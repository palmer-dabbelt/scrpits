...
class SourceHighlightStex
	...
	def SourceHighlightStex.cmds(path)
		...
		out.push("source-highlight -n #{path.chomp(".stex")} -f latexcolor -s #{LANG_MAP[pathsplit(".")[-1]]}")
...
class GIFImagePDF
	...
	def GIFImagePDF.deps(path)
		out = SVGImagePDF.more(path)
		
		out.push("#{path.chomp(".pdf")}.gif")
		
		return out
	end
	
	def GIFImagePDF.cmds(path)
		out = Array.new
		
		out.push("convert #{path.chomp(".pdf")}.gif #{path.chomp(".pdf")}.pdf")
		
		return out
	end
	
	def GIFImagePDF.created(path)
		out = Array.new
